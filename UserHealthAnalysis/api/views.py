...
def lifestyle_planner(request):
    """
    Generate a personalized lifestyle plan based on user health metrics.
    Expects a JSON payload with keys: avg_exercise_per_week, avg_sleep_hours, stress_level, diet_quality, most_freq_substance_used.
    """
    data = request.data
    ...
    prompt = (
        ...
        "Provide recommendations on exercise, sleep, nutrition, and stress management in a clear, actionable format."
    )
    ...
    try:
        ...
        # The structure of the response depends on the model; for text generation, you'll typically have:
        # result[0]["generated_text"] if successful
        generated_text = result[0]["generated_text"] if isinstance(result, list) and "generated_text" in result[0] else "No output generated."
